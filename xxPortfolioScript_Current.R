...
sigma <- sigma[,-1]
Trees <- c("Bl","Cw","Fd","Hw","Lw","Pl","Sx") ##set species to use in portfolio
nSpp <- length(Trees)
treeList <- Trees
...
nsuitF <- file.choose() ##Import species suitability table
SuitTable <- read.csv(nsuitF, stringsAsFactors = FALSE)
SuitTable <- unique(SuitTable)

colnames(SuitTable)[2] <- "SS_NoSpace"

SIBEC <- read.csv("PredSIforPort.csv", stringsAsFactors = FALSE)# SIBEC values - Species by Site Series
SIBEC <- SIBEC[,-5]
colnames(SIBEC)[c(1,3,5)] <- c("SS_NoSpace", "MeanPlotSiteIndex","TreeSpp")

SSPredAll <- read.csv("SiteSeries_Predicted_Bulkley.csv", stringsAsFactors = FALSE) ##Import SS predictions from CCISS tool: must have columns MergedBGC, Source, SS_NoSpace, SSprob, SSCurrent, FuturePeriod, SiteNo
...
SSPredAll <- SSPredAll[SSPredAll$SiteNo %in% sample(sites, 10, replace = FALSE),]# sets number of sample sites
