...
#selectBGC <- "CDFmm"
#SSPredAll <- SSPredAll[grep(selectBGC,SSPredAll$SSCurrent),]
...
outAll <- foreach(BGC = BGCs, .combine = c) %do% {
  cat("########## Proccessing", BGC,"###################\n")
  SSPredBGC <- SSPredFull[SSPredFull$CurrBGC == BGC,]
  SSList <- unique(SSPredBGC$SSCurrent)
  foreach(selectBGC = SSList, .combine = c) %do% {
    cat("########## Proccessing", selectBGC,"###################\n")
