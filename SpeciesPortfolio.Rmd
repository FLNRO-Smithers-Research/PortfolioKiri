...
if(dir.exists("E:/Sync")){
  cloud_dir <- "E:/Sync/Portfolio_Work/"
}else{
...
if(file.exists(paste0(cloud_dir,"CCISSPred.Rdata"))){
  ...
  load(paste0(cloud_dir, "WNAv12_Subzone_19_Var_ranger_17Jan21.Rdata"))
  Edatope <- fread("./InputsGit/Edatopic_v11_20.csv",data.table = F)
  rawDat <- fread(paste0(cloud_dir,inputDatName),data.table = F)
...
analUnits <- fread("./InputsGit/AnalysisUnits.csv")
...
### Species trajectories for zonal sites

```{R CCISS Traj, fig.height = 13, fig.width = 12}
SSPred <- as.data.table(CCISSPred[[1]])
SSPred <- SSPred[grep("01",SSCurrent),]
SSPred[,SiteNo := as.numeric(SiteNo)]
SSPred <- analUnits[SSPred, on = c(ID1 = "SiteNo")]
Curr <- SSPred[,.(SS_NoSpace = unique(SSCurrent),SSprob = 1, FuturePeriod = 2000),
               by = .(BGC_analysis)]
SSPred <- rbind(SSPred,Curr, fill = T)

#SuitShort <- SuitTable[Spp == "Fd",]
SuitShort <- SuitTable[Spp %chin% SppSI,]
SSPred <- SuitShort[SSPred, on = "SS_NoSpace", allow.cartesian = T]
SSPred[is.na(Suitability),Suitability := 5]

#SSPred <- SSPred[BGC_analysis == "MHmm2_lo",]
temp <- SSPred[,.(Prop = sum(SSprob)), by = .(Spp,BGC_analysis, FuturePeriod,Suitability)]
temp <- temp[,.(Prop = Prop/sum(Prop), Suitability), by = .(Spp,BGC_analysis,FuturePeriod)]
temp[,Suitability := as.factor(Suitability)]
t2 <- temp[Suitability %in% c(1,2),.(mProb = max(Prop)), by = .(Spp,BGC_analysis)]
t2 <- t2[mProb >= 0.5,]
setkey(temp, BGC_analysis, Spp)
setkey(t2, BGC_analysis, Spp)
temp <- temp[t2]

ggplot(temp, aes(x = FuturePeriod, y = Prop, fill = Suitability))+
  geom_bar(stat = "identity", width = 0.9)+
  facet_wrap(.~BGC_analysis + Spp)
...
loopCombine <- function(a,b){
  df1 <- rbind(a$GraphDat,b$GraphDat)
  df2 <- rbind(a$MaxS,b$MaxS)
  return(list(GraphDat = df1, MaxS = df2))
}
...
simulateGrowth <- function(x, nYears = modYears){ ###remove any not suitable in any time period
  if(any(x < 0) || max(x) < 0.6){return(NULL)}
    ...
    annualDat <- data.frame("Year" = seq(2000,2000+nYears,1), "Growth" = growthRate, 
                            "MeanDead" = pDead, "NoMort" = rep(25, nYears+1)) ##create working data
...
params <- c(5.0e-07,14.5)

modelYears <- 60
modPeriod <- c(2025,2055)

###read in CBST data
Trees <- c("Ba", "Bl","Cw","Dr", "Fd","Hw","Lw","Pl","Py","Sx") ##set species to use in portfolio
allSppDat <- fread(paste0(cloud_dir,"PortfolioFiles/FilledCBSTNoMigration_v2.csv"))
...
SSPredAll <- CCISS_cbst(rawDat,BGCmodel) %>% as.data.table()
colnames(analUnits)[1] <- "SiteNo"
SSPredAll <- analUnits[SSPredAll, on = "SiteNo"]
SSPredAll <- unique(SSPredAll)
...
```{r runCBST}
Units <- unique(SSPredAll$BGC_analysis)
outAllCBST <- foreach(AU = Units, .combine = loopCombine) %do% {
  SSPredU <- SSPredAll[BGC_analysis == AU,]
  SppRes <- as.data.table(outAll$MaxS)
  SppRes <- SppRes[Unit == AU & !Spp %chin% c("Sd","RealRet"),]
  SppUse <- SppRes[SetRet > 0.25, Spp]
  SL = unique(SSPredU$SiteNo)
  foreach(SppCurr = SppUse,.combine = loopCombine) %do% {
    SuitTable <- allSppDat[Spp == SppCurr,-"Spp"]
    ...
    SSPred <- SSPredU[SSPredU$SiteNo == SNum,]
    currBGC <- as.character(SSPred$BGC[1])
    
    SSPred <- merge(SSPred,SuitTable, by.x = "BGC.pred", by.y = "Site", all.x = TRUE)
    SSPred$GWHeight <- gs2gw(SSPred$Height, as.numeric(params[1]), as.numeric(params[2]))
    SS.sum <- dcast(SSPred, Seed~FuturePeriod, value.var = "GWHeight", fun.aggregate = mean)
    SS.sum <- SS.sum[`2025` > 0.3,]
    SS.sum <- melt(SS.sum, id.vars = "Seed")
    colnames(SS.sum)[2:3] <- c("FuturePeriod","Height")
    
    SeedList <- unique(SS.sum$Seed)
    returns <- data.frame(Year = seq(2000,2000+modelYears,1))
    modData <- data.table()
    
    for(seed in SeedList){
      htDat <- SS.sum[Seed == seed,-1]
      curr <- data.frame(FuturePeriod = 2000, 
                         Height = SuitTable$Height[SuitTable$Site == currBGC & 
                                                     SuitTable$Seed == as.character(seed)])
      htDat <- rbind(curr, htDat)
      
      Returns <- simulateGrowth(htDat$Height, nYears = modelYears)
      if(is.null(Returns)){next}
      tmpR <- c(0,Returns)
      assets <- Returns - tmpR[-length(tmpR)]
      dat <- data.frame(Seed = rep(seed,modelYears+1), Year = 1:(modelYears+1), Returns = assets)
      modData <- rbind(modData, dat, fill = T)
    }
    ...
    boundDat <- data.table(Spp = colnames(returns))
    boundDat[,`:=`(minWt = 0, maxWt = 1)]
    ...
    allSites <- as.data.table(allSites)
    allSites <- dcast(allSites, Return ~ variable, fun.aggregate = function(x){sum(x)/(length(SL))})
    
    maxSharpe <- allSites[allSites$Sharpe == max(allSites$Sharpe),-c("Return","Sharpe")]
    maxSharpe <- t(maxSharpe) %>% as.data.frame() %>% 
      mutate(Spp = rownames(.)) %>% set_colnames(c("Weight","Seed"))
    maxSharpe <- maxSharpe[!maxSharpe$Seed %in% c("Sd","RealRet"),] %>% as.data.table()
    maxSharpe <- maxSharpe[Weight > 0.04,]
    maxSharpe[,`:=`(Weight = Weight/sum(Weight),Unit = AU, Spp = SppCurr)]
    
    temp <- colMeans(allSites[,-c("Return","Sd","RealRet","Sharpe")])
    temp <- names(temp[temp > 0.04])
    varDat <- allSites[,..temp]
    idDat <- allSites[,.(Return,Sd)]
    allSites <- cbind(idDat,varDat)
    allSites <- melt(allSites, id.vars = c("Return","Sd"))
    allSites <- allSites[,.(Weight = value/sum(value), Seed = variable), by = .(Return, Sd)]
    allSites[,`:=`(Unit = AU, Spp = SppCurr)]
    list(GraphDat = allSites, MaxS = maxSharpe)
  }
}
...
#### Efficient frontier plots for CBST portfolio

```{r plotCBST, fig.height = 13, fig.width = 12}
efAll <- as.data.table(outAllCBST$GraphDat)
efAll <- efAll[complete.cases(efAll),]

ggplot(efAll,aes(x = Sd, y = Weight,group = Seed))+
  geom_area(aes(fill = Seed), size = 0.00001, col = "grey50", stat = "identity")+
  scale_x_reverse() +
  xlab("Max Return --> Minimized Risk")+
  ylab("Portfolio Ratio")+
  theme_few()+
  facet_wrap(.~Unit+Spp, scales = "free_x")
```

#### Max Sharpe ratio for CBST portfolio

```{r CBST_maxS,fig.width = 8.5, fig.height = 9}
dat <- outAllCBST$MaxS
ggplot(dat, aes(x = Spp, y = Weight, group = Seed, fill = Seed))+
  geom_bar(stat = "identity")+
  facet_wrap(.~Unit, scales = "free_x")+
  theme_few()
```
