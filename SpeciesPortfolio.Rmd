...
ggplot(efAll[efAll$variable != "RealRet",],aes(x = Sd, y = value,group = variable))+
  ...
  colScale +
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = value,colour = "blue"), linetype = "twodash", size = .5)+
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = SetRet,colour = "grey27"),linetype = "dashed", size = .5)+
...
ggplot(BGCPred, aes(x = BGC_analysis,y = Prop, fill = BGC.pred, label = BGC.pred))+
  geom_bar(stat = 'identity')+
  geom_text(size = 2, position = position_stack(vjust = 0.5), angle = 20)+
  theme_few()+
...
```{r SI_check, fig.width = 10, fig.height = 14, fig.cap = "Figure 5. Mean and 95% CI for SI within Spp and Period, weighted by SSprob"}
...
gr <- foreach(unit = unique(SSPredFull$BGC_analysis),.combine = c) %do% {
  ...
  sub[,SI := mean(MeanPlotSiteIndex), by = .(FuturePeriod, TreeSpp)]
  sub[,CI_min := CI(MeanPlotSiteIndex)[3], by = .(FuturePeriod, TreeSpp)]
  sub[,CI_max := CI(MeanPlotSiteIndex)[1], by = .(FuturePeriod, TreeSpp)]
