...
locations2 <- locations %>% dplyr::rename(latitude = "ycoord", longitude = "xcoord", elevation = "elevation_1") %>% dplyr::select (id, BGC_analysis,latitude,longitude)
...
##############
ft2 <- flextable(boundDat2) %>% set_caption("Minimum and max percent of portolio weighting for individual species in portfolio")
...
>>>>>>> 0c5e738d5279e363eead29f04bc372dc186a37c2

Graphs for each analysis unit based on a rotation period to 2070 showing the efficient frontier species ratios across risk/return choices. 


```{r plot_pf, fig.height = 13, fig.width = 12, fig.cap = "Efficient frontiers for future portfolios by analysis unit"}
pdfOutput <- F
if(pdfOutput){
...
ggplot(efAll[efAll$variable != "RealRet",],aes(x = Sd, y = value,group = variable))+
  ...
  colScale +
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = value,colour = "blue"), linetype = "twodash")+
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = SetRet,colour = "grey27"),linetype = "dashed")+
...
+ The table below shows the min and max values for each species in the portfolio
`r ft2`
