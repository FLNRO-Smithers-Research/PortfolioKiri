...
# boundDat2 <-  boundDat %>%  pivot_wider() %>% as.tibble()
# boundDat2$Spp <- as.factor(boundDat2$Spp)
# boundDat3 <- boundDat2 %>% pivot_wider(names_from = Spp), values_from = c(minWt, maxWt)) #dplyr::select (-name) %>% group_by (Spp)  %>% as.data.frame()  
