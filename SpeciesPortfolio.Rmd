...
boundDat2 <-  data.table::transpose(as.data.frame(boundDat))%>% row_to_names (row_number = 1)
...
Species with less than weight < `r minAccept*100`% of final portfolio will be removed.
    ...
    allSitesSpp <- foreach(SNum = SL, .combine = rbind, 
                       ...
                       .noexport = c("simGrowthCpp")) %do% {
                         ...
                         if(length(timePeriods) == 1){
                           temp <- SS.sum
                           temp$FuturePeriod <- SS.sum$FuturePeriod[1]+55
                           SS.sum <- rbind(SS.sum, temp)
                         }
                         
                         if(!is.null(SS.sum)){
                           annualDat <- data.frame("Year" = seq(2000,2070,1))
                           ...
                           for (k in 1:nSpp){ ##for each tree
                             ...
                             s <- approx(dat$Period, dat$SIBEC, n = 71) ##Smooth SI
                             p <- approx(dat$Period, dat$ProbDead, n = 71) ###Smooth Prob Dead
                             m <- approx(dat$Period, dat$NoMort, n = 71) ##Smooth No Mort
                             
                             ###data frame of annual data
                             annualDat <- data.frame("Year" = seq(2000,2070,1), "Growth" = s[["y"]], 
                                                     "MeanDead" = p[["y"]], "NoMort" = m[["y"]]) ##create working data
                             
                             Returns <- simGrowthCpp(DF = annualDat)
                             tmpR <- c(0,Returns)
                             assets <- Returns - tmpR[-length(tmpR)]
                             temp <- data.frame(Spp = rep(treeList[k],71), 
                                                Year = 1:71, Returns = Returns)
...
## Portfolio plots for each analysis unit based on a rotation period to 2070showing the efficient frontier species ratios across risk/return choices. 
