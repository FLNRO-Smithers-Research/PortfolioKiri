...
load(paste0(cloud_dir_will, "WNAv11_35_VAR_SubZone_ranger.Rdata"))

Edatope <- fread("./InputsGit/Edatopic_v11_20.csv",data.table = F)
rawDat <- fread(paste0(cloud_dir_will,"PortfolioFiles/SunshPts_90 GCMsMSY.csv"),data.table = F)
...
```{r location map}
locations <- fread("./InputsGit/SunshineCoast_SamplePoints_New.csv", data.table = F) 
...
BGCmap <- readOGR("inputs/SunshineCoast.gpkg", "BGC_Dissolved")
# BGCmap@data$id <- rownames(BGCmap@data)
# BGCmap <- fortify(BGCmap, region = "id")## creates dataframe version of shape file for ggplot
# BGCmap <- join(BGCmap, BGCmap@data, by = "id")  ## joins back the data
###create spatial layer of sampled mountains for overlay
coordinates(locations2)<- ~ longitude + latitude  ##make as SpatialPointsDataFrame"
proj4string(locations2)<- CRS("+proj=longlat +datum=NAD83") ## add current projection
locations2 <-spTransform(locations2, CRS(proj4string(BGCmap))) ### change projection to match Provincial Outline
#identical(proj4string(provmap),proj4string(mnt_georef3))# double check that they match
locations <- as.data.frame(locations2) # convert back to data.frame for ggplot
...
Need to output table of these variables
```{r model parameters}
