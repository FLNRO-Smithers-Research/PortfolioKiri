  ...
  SSout <- foreach(selectBGC = SSList, .combine = loopCombine) %do% {
    ...
    if(!is.null(allSitesSpp)){
      ...
      p1 <- ggplot(efAll[efAll$variable != "RealRet",],aes(x = Sd, y = value,group = variable))+
        ...
        geom_line(data = efAll[efAll$variable == "RealRet",], aes(x = Sd, y = value), colour = "black",linetype = "F1", size = .75)+
        scale_x_reverse() +
...
## Portfolio plots for each analysis unit based on a rotation to 2070  showing the efficient frontier species ratios across risk/return choices. 
...
This should probably SI averaged over al future scenarios. Could also have CI bars on each bar.
