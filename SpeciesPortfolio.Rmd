...
ggplot(efAll[efAll$variable != "RealRet",],aes(x = Sd, y = value,group = variable))+
  ...
  colScale +
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = value,colour = "blue"), linetype = "twodash", size = .75)+
  geom_vline(data = intDat[intDat$Spp == "Sd",], aes(xintercept = SetRet,colour = "grey52"),linetype = "dashed", size = .75)+
  ...
  SSout <- foreach(selectBGC = SSList, .combine = loopCombine) %do% {
    ...
    allSitesSpp <- foreach(SNum = SL, .combine = rbind, 
                       ...
                       .noexport = c("simGrowthCpp")) %do% {
                         ...
                         #cat("Optimising site",SNum,"...\n")
                         SS.sum <- cleanData(SNum, Trees, timePer = timePeriodsCurr)
                         SS.sum$FuturePeriod <- as.numeric(SS.sum$FuturePeriod)
                         if(length(timePeriodsCurr) == 1){
                           temp <- SS.sum
                           temp$FuturePeriod <- SS.sum$FuturePeriod[1]+85
                           SS.sum <- rbind(SS.sum, temp)
                         }
