...
This report presents a range of risk-return optimized portfolios of species for reforestation generated using Markowitz Modern Portfolio Theory (MPT). The analysis uses the forecast site series level changes to species feasibility from the Climate Change-Informed Species Selection (CCISS) tool and site index by site series growth information (SIBEC) to calculate the optimal mix of tree species given the modelled uncertainty in future climates up to 2070.
The model uses the CCISS model to forecast future BGCs for 30 GCM/carbon scenarios for all locations and aligns the current site series with future potential site series via edatopic overlap. A MPT portfolio is constructed for each future climate using data on tree species suitability, site index by site series information, and covariance of species in all current and potenial future site series. Probabalistic loss rates are modelled based on species suitability. 
User inputs to the script are training point locations that will represent BGC-based analysis units (e.g. MHmm1_high and MHmm1_low). 
This report is generated from Rmarkdown.
...
##############
ft2 <- flextable(boundDat) %>% set_caption("Min and max weights in portfolio")
...
timePeriods = c(2000,2025,2055,2085)
...
outAll <- foreach(BGC = Units, .combine = loopCombine,
  ...
  SSout <- foreach(selectBGC = SSList, .combine = loopCombine) %do% {
                       ...
                       .noexport = c("simGrowthCpp")) %do% {
                         ...
                         if(!is.null(SS.sum)){
                           ...
                           for (k in 1:nSpp){ ##for each tree
                             ...
                             dat <- data.frame("Period" = rescale(as.numeric(DatSpp$FuturePeriod), to = c(2000,2085)), 
    ...
    if(!is.null(allSitesSpp)){
      ...
      RetCurve <- approx(efAll$RealRet,efAll$Sd,xout = 0.9)
      ret90 <- RetCurve$y
      ...
      p1 <- ggplot(efAll[efAll$variable != "RealRet",],aes(x = Sd, y = value,group = variable))+
        geom_area(aes(fill = variable), size = 0.00001, col = "black", stat = "identity")+
        colScale +
        geom_vline(xintercept = maxSPos, linetype = "dashed")+
        geom_vline(xintercept = ret90, colour = "#040404",linetype = "dashed")+
...
Plot efficient frontier of each portfolio

```{r plot_pf, fig.height = 15, fig.width = 12}
pdfOutput <- TRUE
if(pdfOutput){
...
gr <- foreach(unit = unique(BGCPred$BGC_analysis), .combine = c) %do% {
  temp <- BGCPred[BGC_analysis == unit,]
...
## Estimated site index of portfolio species in each analysis unit for each time period
