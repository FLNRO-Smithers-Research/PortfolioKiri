...
pemPort <- foreach(poly = unique(pemData$PolyID)[1:10], .combine = loopCombine2) %do% {
...
st_write(pemOut, dsn = paste0(cloud_dir,"PortfolioFiles/QuesnelPresentation"), 
...
avg <- colMeans(dat[,-1])
dat <- pemArea[dat, on = c(Polygon = "PolyLab")]
ft1 <- flextable(dat) %>% colformat_num(digits = 2) %>% set_caption("Portfolio results for each polygon")
ft1

dat2 <- data.table(Spp = names(avg),Weight = avg)
dat2 <- dat2[Weight >= 0.05, .(Spp = Spp,Weight = Weight/sum(Weight))]
dat2[,Area := Weight*sum(dat$AREA)]
dat2[,NumTrees := Area/2.2]
