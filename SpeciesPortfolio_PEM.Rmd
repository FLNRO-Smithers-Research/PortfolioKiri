...
pem <- st_read(dsn = paste0(cloud_dir,"PortfolioFiles/QuesnelPem"), layer = pem_name)
pemBGC <- unique(paste0(pem$Bgc_Zone,pem$Bgc_Subzon,pem$Bgc_Vrt))
pem <- pem[pem$Site_S1 != 0, c("AREA","PEM_QU_ID","Sdec_1","Site_S1","Sdec_2","Site_S2","Sdec_3","Site_S3")]
pem <- data.table(pem)
pem <- pem[,.(geometry = st_union(geometry)), by = .(Sdec_1,Site_S1,Sdec_2,Site_S2,Sdec_3,Site_S3)]
...
pemPort <- foreach(poly = unique(pemData$PolyID), .combine = rbind) %do% {
...
setorder(dat, -Weight)
dat <- dat[1:5,]
temp <- data.frame(V1 = c(dat$Spp,dat$Weight))
rownames(temp) <- c("Spp1","Spp2","Spp3","Spp4","Spp5","Wt1","Wt2","Wt3","Wt4","Wt5")
...
```{r add to pem}
pemOut <- st_sf(data.frame(pemPort,pem))
st_write(pemOut, dsn = paste0(cloud_dir,"PortfolioFiles/QuesnelPem"), 
         layer = "WithPortfolio", driver = "ESRI Shapefile")
```
